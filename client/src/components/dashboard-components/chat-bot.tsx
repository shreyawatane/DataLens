  ...
  return (
    ...
    >
      <div className={`flex ${message.isBot ? 'flex-row' : 'flex-row-reverse'} max-w-[80%] items-start gap-2`}>
        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${message.isBot ? 'bg-green-100' : 'bg-green-600'}`}>
          {message.isBot ? <Bot className="w-5 h-5 text-green-600" /> : <User className="w-5 h-5 text-white" />}
        </div>
        <div className={`flex flex-col ${message.isBot ? 'items-start' : 'items-end'}`}>
          <div className={`p-3 rounded-lg ${message.isBot ? 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200' : 'bg-green-600 text-white'}`}>
            {message.isBot ? (
              <ReactMarkdown
...
export const Chatbot = () => {
  ...
  const inputRef = useRef<HTMLInputElement>(null); // Add local inputRef
  ...
  return (
    <div>
      <button
        ...
        className="fixed bottom-16 right-4 bg-green-600 hover:bg-green-700 text-white p-4 rounded-full shadow-lg transition-colors duration-200 z-50"
      >
        ...
        >
          ...
          <div className="p-2 md:p-4">
            ...
            <div className="mt-2 md:mt-4 flex">
				...
				ref={inputRef}  // Use local inputRef
				/>
              <button
                onClick={sendMessage}
                disabled={isLoading}
